    ...
    io.on('connection', socket => {
        ...
        socket.on('send message', async (message) => {
            ...
            if (message.msg.txt.toLowerCase().includes('?')) {
                ...
                if (!messagesFound || messagesFound.length === 0) return 
                console.log('messagesFound.length',messagesFound.length)
                let lastMessagesIndex = messagesFound.length-1
                let messagefound = {
                    msg: { 
                        name:'bot',
                        txt: messagesFound[lastMessagesIndex]._source.answer
                    },
